...
const SPLASH_CONFIG = {
    EMISSION_RATE: 0.005,      // Emit particles much more frequently (was 0.01)
    PARTICLE_COUNT: 15,        // Many more particles per emission (was 6)
    PARTICLE_SIZE: 0.8,        // Much larger particles (was 0.5)
    PARTICLE_COLOR: 0x55aaff,  // Brighter blue color for better visibility
    PARTICLE_LIFETIME: 2.5,    // Much longer lifetime (was 1.2)
    PARTICLE_OPACITY: 0.9,     // Higher opacity for better visibility (was 0.8)
    PARTICLE_SPEED: 0.5,       // Faster movement (was 0.3)
    RANDOM_SPEED: 0.4,         // More random variation (was 0.2)
    SPLASH_HEIGHT: 2.5,        // Much higher splash (was 1.2)
    GRAVITY: 9,                // Reduced gravity for slower falling (was 12)
    EFFECT_DURATION: 3.0,      // How long effects persist after dragging stops (was 1.0)
    SPAWN_RADIUS: 3.0,         // How far from monster center particles can spawn
    INITIAL_SPLASH_COUNT: 30   // Number of particles in initial splash (was 15)
};
...
function createImmediateSplash(monster) {
    ...
    // Create MANY more particles in a big splash
    for (let i = 0; i < SPLASH_CONFIG.INITIAL_SPLASH_COUNT; i++) {
        // Larger size with more variation
        ...
        // Higher, more dramatic velocity
        const velocity = new THREE.Vector3(
            (Math.random() - 0.5) * 6,  // More horizontal spread
            3 + Math.random() * 6,      // Much higher upward velocity
            (Math.random() - 0.5) * 6   // More horizontal spread
        );
    ...
    monsters.forEach(monster => {
        ...
        if (isDragged) {
            ...
            if (!dragEffects.has(monster)) {
                ...
                // Create splash particles regardless of movement (modified)
                // Remove the distance check to ensure constant particles
...
function createSplashParticles(monster, currentTime, deltaTime, distanceMoved) {
    ...
    for (let i = 0; i < particleCount; i++) {
        // Create particle with larger size variation
        ...
        // Calculate velocity with direction based on monster's movement
        // and random variation for natural splash appearance
        let velocityX = (Math.random() - 0.5) * SPLASH_CONFIG.RANDOM_SPEED * 2;
        let velocityZ = (Math.random() - 0.5) * SPLASH_CONFIG.RANDOM_SPEED * 2;

        // Add component from monster movement direction (40% influence)
        if (velocity.length() > 0.1) {
            velocityX += velocity.x * 0.4;
            velocityZ += velocity.z * 0.4;
        }

        // Higher upward velocity based on monster speed
        const upwardSpeed = SPLASH_CONFIG.SPLASH_HEIGHT * (0.7 + Math.random() * 0.6) * speedFactor;
        const particleVelocity = new THREE.Vector3(velocityX, upwardSpeed, velocityZ);
...
function updateParticles(deltaTime) {
    ...
    dragEffects.forEach(effects => {
        for (let i = effects.particles.length - 1; i >= 0; i--) {
            ...
            // Apply gravity to particle - stronger gravity
...
export function createDebugSplash(position) {
    ...
    for (let i = 0; i < 40; i++) {
        ...
        // Random velocity but mostly upward, more dramatic
        const velocity = new THREE.Vector3(
            (Math.random() - 0.5) * 4,
            4 + Math.random() * 4,
            (Math.random() - 0.5) * 4
